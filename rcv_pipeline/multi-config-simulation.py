...
# A file specific to configuration scenarios for MA. 
# I created 
...
# raw ones.
configs = glob.glob(f"configurations/{location}/{config_str}/config-{MAG_LIST[0]}-{MAG_LIST[1]}-{MAG_LIST[2]}.json")
...
for config_name in configs:
  ...
  # Write to file!
  write = Path(f"./output/results/{location}/{location}-{MAG_LIST[0]}-{MAG_LIST[1]}-{MAG_LIST[2]}-{config_str}")
